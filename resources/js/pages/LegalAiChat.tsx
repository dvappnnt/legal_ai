...
export default function LegalAiChat() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-100 p-4">
      <div className="flex flex-col bg-white shadow-2xl rounded-lg w-full max-w-2xl h-[600px]">
        <div className="px-6 py-4 border-b flex justify-between items-center bg-[#10214B] text-white rounded-t-lg">
          <h1 className="text-xl font-bold">A.I. delas alas</h1>
          <div className="space-x-2">
            <button onClick={handleClear} className="px-3 py-1 bg-[#d7bd88] rounded hover:bg-[#D0c3ba] text-sm text-[#10214B]">
              Clear
            </button>
            <button onClick={handleCopy} className="px-3 py-1 bg-[#d7bd88] rounded hover:bg-[#D0c3ba] text-sm text-[#10214B]">
              Copy Answer
            </button>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-4">
          ...
          {loading && messages.length > 0 && messages[messages.length - 1].sender === 'user' && (
            <div className="w-1/16 p-3 rounded-lg mr-auto bg-gray-200 text-gray-800 italic opacity-70">
              {`•${typingDots}`}
            </div>
          )}
        ...
        <div className="p-4 border-t flex flex-col space-y-2">
          ...
          <button
            onClick={handleAsk}
            disabled={loading}
            className="bg-[#10214B] text-white px-4 py-2 rounded hover:bg-[#D0c3ba] disabled:opacity-50 self-end hover:text-[#10214B]"
          >
